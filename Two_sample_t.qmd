...
3.  $(H_0)$: the mean of group A $(m_A)$ is less than or eaqual to the mean of group B $(m_B)$- one tailed test.
...
$S^2 = { {\sum{ (x-m_{A})^2} + \sum{ (x-m_{B})^2}} \over {n_{A} + n_{B} - 2 }}$
...
If the data is not independent such as paired data in the form of matched pairs which are correlated, we use the *paired t test*. This test checks whether the means of two paired groups are different from each other. It usually used in clinical trial studies with a "before and after" or case control studies with matched pairs. For this test we only assume the difference of each pair to be normally distributed (the paired groups are important for analysis) unlike the *independent t test* which assumes that data from both samples are independent and variances are equal.[@fralick]
...
$H_1$: There is a difference between the final grades between the two groups.
...
male = math$G3[math$sex == "M"]
female = math$G3[math$sex == "F"]

# plotting distribution for males
plotNormalHistogram(male)

```

Final grades for males seem to be normally distributed from 0-20.

```{r}
#| echo: true 

# plotting distribution for females
plotNormalHistogram(female)
...
sample_size = table(math$sex)
barplot(sample_size,main= "Distribution of sample size by sex")
```

The bar plot shows that there are slightly more females in the sample than males.
...
boxplot(G3 ~ sex,data=math)
```

The box plot shows us that there are outliers in terms of people who had a score of 0. This score is not truly reflective of the performance between boys and girls as a grade of 0 may represent absentia or other reasons for the test not been taken. Therefore we opt to drop the outliers. We will compare to see if this decision affects the mean which appears similar from the above plot.
...
The mean has increased slightly but the distribution is still the same.

**Check the equality of variances (homogeneity)**

We can use the *Levene's test* or the *Bartlett's test* to check for homogeneity of variances. The former is in the [car]{style="background-color:yellow"} library and the later in the [rstatix]{style="background-color:yellow"} library. If the variances are homogeneous the p value will be greater than 0.05.
...
$\color{pink}{\text{gtsummary}}$ table of results
...
For the two sample t test with t(355) = -1.940477, p \< 0.0531, the p value is greater than our alpha of 0.05 , we fail to reject the null hypothesis and conclude that there is no difference between the means of the two groups. There is no difference in final grades between boys and girls.
...
In conclusion, a two-samples t-test showed that the difference was not statistically significant, t(355) = -1.940477, p \< 0.0531, d = -0.20; where, t(355) is shorthand notation for a t-statistic that has 355 degrees of freedom and d is *Cohen's d*.
...
The p value \> 0.05 (p=0.973) , we fail to reject the null hypothesis. We conclude that the final grades for females are not significantly greater than for males.
...
1. Cortez, Paulo, and Alice Maria Gonçalves Silva. 2008. “Using data mining to predict secondary school student performance.” In. EUROSIS-ETI. [http://repositorium.sdum.uminho.pt/.](http://repositorium.sdum.uminho.pt/)

2. Cressie, N. a. C., and H. J. Whitford. 1986. “How to Use the Two Sample t-Test.” Biometrical Journal 28 (2): 131–48. [https://doi.org/10.1002/bimj.4710280202.](https://onlinelibrary.wiley.com/doi/10.1002/bimj.4710280202)

3. FRALICK, Drew, Julia Z. ZHENG, Bokai Wang, Xin M. TU, and Changyong FENG. n.d. “The Differences and Similarities Between Two-Sample t-Test and Paired t-Test.” Shanghai Archives of Psychiatry 29 (3): 184–88. [https://doi.org/10.11919/j.issn.1002-0829.217070.](http://www.shanghaiarchivesofpsychiatry.org/Download/Volume%2028/Issue%206/217070en.pdf)
