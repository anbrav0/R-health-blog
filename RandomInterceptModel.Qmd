...
$$ y_i = \beta_0 + X_i\beta_i + \alpha_u + u_{it} $$
...
In this lecture you learned about the importance of a random intercept model, when it is appropriate to use a random intercept model, the difference between an ordinary single-level model, the Interclass correlation coefficient (ICC) and how to interpret results from the fixed part and the random part of a random intercept model. 
