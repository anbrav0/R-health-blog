...
Generalized linear function also follows certain assumptions: 
...
here we can do some further diagnostics as well like plotting the residuals for this first model fit

```{r}

res <- residuals(PossionModel1, type = "deviance")
plot(log(predict(PossionModel1)), res)
abline(h = 0, lty = 2)
qqnorm(res)
qqline(res)
...
ploting the residual (raw deviance or scaled) on a y axis vs the log predicted values on x axis. here are some large residuals and lots of deviance from the norm. (however I have heard that normality of rediauls is not to be expected so this may not be conclusive)
...
> _we can also compare our model to a zero inflated model to check for fit:_
